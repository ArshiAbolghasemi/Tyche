#!/usr/bin/env bash
#
# Portfolio run on FinBERT-MiniLM sentiment, Student-t target distribution.
#   -> benchmark/finbert_minilm/student_t/
#
# Needs ./scripts/news/finbert_minilm.sh to have run first.
# Extra args override the defaults, e.g.
#   ./scripts/portfolio/student_t/finbert_minilm.sh --holdings 5 20

set -euo pipefail
cd "$(dirname "${BASH_SOURCE[0]}")/../../.."

TYCHE_SENTIMENT_BACKENDS=finbert_minilm \
TYCHE_PORTFOLIO_PATHS_NEWS_SENTIMENT=data/output/news_sentiment_finbert_minilm.parquet \
TYCHE_PORTFOLIO_TRAIN_TARGET_DISTRIBUTION=student_t \
  uv run python -m tyche.portfolio.run \
    --holdings 1 2 3 5 10 20 40 60 \
    --transaction-cost-bps 0 1 2 5 10 "$@"
